...
class ChatInterface:
    ...
    def __init__(self, agent, tools_dict):
        ...
        self.browser_image_suffixes = {".jpg", ".jpeg", ".png", ".gif", ".webp"}

    def _prepare_display_image(self, image_path: Path, timestamp: int) -> str:
        """
        Return a browser-friendly image path for the Gradio preview.
        """
        if image_path.suffix.lower() in self.browser_image_suffixes:
            return str(image_path)

        display_path = self.upload_dir / f"upload_{timestamp}_display.png"
        with Image.open(image_path) as image:
            image.convert("RGB").save(display_path, format="PNG")
    ...
    def handle_upload(self, file_path: str) -> str:
        ...
        if suffix == ".dcm":
            ...
            self.display_file_path = self._prepare_display_image(saved_path, timestamp)
    ...
    def add_message(
        ...
        if image_path is not None:
            history.append({"role": "user", "content": {"path": display_image or image_path}})
    ...
    async def process_message(
        ...
        if image_path is not None:
            ...
            multimodal_image_path = self.display_file_path or display_image or image_path
            with open(multimodal_image_path, "rb") as img_file:
...
def create_demo(agent, tools_dict):
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        def handle_file_upload(file):
            return interface.handle_upload(file)
        ...
        image_display.upload(handle_file_upload, inputs=image_display, outputs=image_display)
