...
def find_available_port(start_port: int = 8585, host: str = "0.0.0.0", attempts: int = 50) -> int:
    """Find an available TCP port, starting with the preferred Gradio port."""
...
def initialize_agent(
    prompt_file,
    tools_to_use=None,
    model_dir=os.path.expanduser("~/model-weights"),
    temp_dir="temp",
    device="cuda",
...
if __name__ == "__main__":
    ...
    if os.getenv("MEDRAX_ENABLE_VQA", "").lower() in {"1", "true", "yes"}:
        selected_tools.append("XRayVQATool")
    ...
    agent, tools_dict = initialize_agent(
        ...
        model_dir=os.path.expanduser("~/model-weights"),  # Change this to the path of the model weights
        temp_dir="temp",  # Change this to the path of the temporary directory
        device="cuda",  # Change this to the device you want to use
        model=os.getenv("OPENAI_MODEL", "rfsousa/qwen2.5vl:tools"), # Change this to the model you want to use, e.g. gpt-4o-mini
        temperature=0.7,
        top_p=0.95,
    ...
    server_name = "0.0.0.0"
    server_port = find_available_port(int(os.getenv("GRADIO_SERVER_PORT", "8585")), server_name)
